...
const shouldPreprocess = (filename: Path, config: Config): boolean => {
  ...
  return !!config.transform.length && (
...
const preprocessorCache: WeakMap<
  Config,
...
const getPreprocessor = (filename: string, config: Config): ?Preprocessor => {
  ...
  if (
    ...
    preprocessorCache.get(config).get(ext)
  ) {
    return preprocessorCache.get(config).get(ext);
  } else {
    let preprocessor;
    if (!config.transform.length) {
      ...
      for (let i = 0; i < config.transform.length; i++) {
        if (new RegExp(config.transform[i][0]).test(ext)) {
    ...
    preprocessorCache.get(config).set(ext, preprocessor);
    return preprocessor;
  }
};
